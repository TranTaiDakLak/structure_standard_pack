...
### `shared` — nhiều project nhỏ/vừa trên một server
...
### `dedicated` — project quan trọng hoặc workload vừa/nặng
...
### `isolated` — production lớn hoặc cần cô lập mạnh
...
- compose source of truth rõ ràng: `docker-compose.yml` ở root hoặc `infra/compose/prod.yml` theo template đang dùng
...
- [ ] Có compose production source of truth (`docker-compose.yml` hoặc `infra/compose/prod.yml`), `.env`, `.env.example`, `config/`, `storage/` nếu có runtime file, `README.md`.
...
- [ ] Worker/job bắt đầu làm API chậm hoặc tranh tài nguyên với app khác.
- [ ] DB shared thường xuyên chạm CPU/RAM/connection limit.
- [ ] Backup/restore vượt quá cửa sổ vận hành chấp nhận được.
- [ ] Log/metrics cho thấy lỗi hoặc latency tăng theo lịch job.
- [ ] Dữ liệu hoặc yêu cầu compliance không còn phù hợp với shared infra.
