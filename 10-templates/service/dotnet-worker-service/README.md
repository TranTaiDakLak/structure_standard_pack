...
- Ràng buộc bởi [`03-standards/API_RESPONSE_CONTRACT.md`](../../../03-standards/API_RESPONSE_CONTRACT.md) (contract `api-1`) ở 2 mức. **Mức 1 — hợp đồng với hệ thống chạy service:** `/healthz` + `/readyz` đúng hình dạng khi service có HTTP surface; worker chạy theo lịch rồi thoát thì miễn HTTP và lấy **exit code** làm contract (`0` = mọi bước bắt buộc hoàn tất, `!= 0` = cần người can thiệp), danh sách bước bắt buộc / best-effort khai trong `docs/` — [appendix](../../../03-standards/API_RESPONSE_CONTRACT_APPENDIX.md) section 2.3. `trace_id` của lần chạy là `run_id` sinh lúc bắt đầu, persist cùng bản ghi (appendix section 2.0).
...
- Minimal API nội bộ nếu có thì áp toàn bộ contract như một API thật, kể cả `JsonNamingPolicy.SnakeCaseLower`.
