...
- **Khuyến nghị mạnh cho riêng stack này**: 1 integration test smoke duyệt mọi route đã mount ở `cmd/api/main.go` — một request lỗi cố ý assert body có đủ `error.code` + `error.trace_id`, và một request tới route list assert body có đúng 2 khóa `items` + `page`. Nhiều module cùng viết `handler/` là nơi drift hình dạng dễ xảy ra nhất, và test này bắt được ngay lần thêm module mới đầu tiên. Không bắt buộc — xem chuẩn section 13.
