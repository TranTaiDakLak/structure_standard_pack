...
- **Rule cứng 2 (không phải gợi ý)**: bắt buộc 1 integration test smoke ở `apps/api/tests/integration/` duyệt MỌI route đã mount ở `internal/app/http.go` — một request lỗi cố ý assert body có đủ `error.code` + `error.trace_id`, **và một request tới route list assert body có đúng 2 khóa `items` + `page`**. Nhánh list là chỗ duy nhất test này chạm success; thiếu nó thì drift hình dạng success không ai bắt được (chuẩn section 13). Test rẻ và bắt được drift ngay lần thêm module mới đầu tiên.
